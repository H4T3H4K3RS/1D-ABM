...
def make_event_df(info: Any = None, config: Dict = None):
    ...
    for event in events:
        ...
        Logger.info(f"Event: {event}. State it: {state_it}")
        is_recovered = False
        c = 0
        ...
        last_ask_orders_count = info_dict['orders'][-1]['quantity']['ask']
        last_bid_orders_count = info_dict['orders'][-1]['quantity']['bid']
        event_ask_orders_count = info_dict['orders'][event['it']]['quantity']['ask']
        event_bid_orders_count = info_dict['orders'][event['it']]['quantity']['bid']
        avg_ask_before_event = math.mean(
            [info_dict['orders'][i]['quantity']['ask'] for i in range(event['it'] - 10, event['it'])])
        avg_bid_before_event = math.mean(
            [info_dict['orders'][i]['quantity']['bid'] for i in range(event['it'] - 10, event['it'])])
        avg_ask_after_event = math.mean(
            [info_dict['orders'][i]['quantity']['ask'] for i in range(event['it'], event['it'] + 10)])
        avg_bid_after_event = math.mean(
            [info_dict['orders'][i]['quantity']['bid'] for i in range(event['it'], event['it'] + 10)])
        avg_diff_before_event = abs(avg_ask_before_event - avg_bid_before_event)
        avg_diff_after_event = abs(avg_ask_after_event - avg_bid_after_event)
        data.append({
            # "type": event['type'],
            **event,
            **traders,
            ...
            'last_ask_orders_count': last_ask_orders_count,
            'last_bid_orders_count': last_bid_orders_count,
            'event_ask_orders_count': event_ask_orders_count,
            'event_bid_orders_count': event_bid_orders_count,
            'diff_ask_orders_count': last_ask_orders_count - event_ask_orders_count,
            'diff_bid_orders_count': last_bid_orders_count - event_bid_orders_count,
            'avg_diff_before_event': avg_diff_before_event,
            'avg_diff_after_event': avg_diff_after_event,
