class Order:
    ...
    **better-offer < worse-offer**
    ...
    def __init__(self, price, qty, order_type, it, trader_link=None):
        # Properties
        self.price = price
        self.qty = qty
    ...
    def __repr__(self) -> str:
        return f'{self.order_type} (price={self.price}, qty={self.qty}, it={self.it})'

    def to_dict(self) -> dict:
        return {'price': self.price, 'qty': self.qty, 'order_type': self.order_type,
                'it': self.it, 'trader_link': self.trader}

    @classmethod
    def from_dict(cls, order_dict):
        return Order(order_dict['price'], order_dict['qty'], order_dict['order_type'], order_dict['it'],
...
class OrderList:
    ...
    def __len__(self):
        return self.filter_length(int(1e12))

    def to_list(self) -> list:
        return [order.to_dict() for order in self]

    def filter_list(self, it: int) -> list:
        return [order.to_dict() for order in self if order.it <= it]

    def filter_length(self, it: int) -> int:
        return sum(1 for order in self if order.it <= it)
    ...
    def from_list(cls, order_list, sort=False):
        try:
            order_list = [Order(order['price'], order['qty'], order['order_type'], order['it'],
                                order.get('trader_link')) for order in order_list]
        except TypeError:
            order_list = [Order(order.price, order.qty, order.order_type, order.it, order.trader) for order in
                          order_list]
