...
class ExchangeAgent:
    ...
    def _fill_book(self, price, std, volume, div: float = 0.05):
        ...
        for (p, q) in zip(sorted(prices1 + prices2), quantities):
            if p > price:
                order = Order(round(p, 1), q, 'ask', self.iteration, None)
                self.order_book[self.iteration]['ask'].append(order)
            else:
                order = Order(p, q, 'bid', self.iteration, None)
                self.order_book[self.iteration]['bid'].push(order)
    ...
    def _clear_book(self):
        ...
        :return: void
        """
        self.order_book[self.iteration]['bid'] = OrderList.from_list(
            [order for order in self.order_book[self.iteration]['bid'] if order.qty > 0])
        self.order_book[self.iteration]['ask'] = OrderList.from_list(
            [order for order in self.order_book[self.iteration]['ask'] if order.qty > 0])

    def spread(self, signal_threshold: int = 0) -> dict or None:
        """
        :return: {'bid': float, 'ask': float}
        """
        # TODO: add iteration to spread
        current_book = {'bid': self.order_book[self.iteration - signal_threshold]['bid'],
                        'ask': self.order_book[self.iteration - signal_threshold]['ask']}
        if current_book['bid'] and current_book['ask']:
            # logging.Logger.info(f"===[{self.iteration} - {signal_threshold}]===")
            # logging.Logger.info(
            #     f'bid: {self.order_book[self.iteration]["bid"].first.price}, ask: {self.order_book[self.iteration]["ask"].first.price}')
            # logging.Logger.info(
            #     f'bid: {current_book["bid"].first.price}, ask: {current_book["ask"].first.price}')
            # logging.Logger.info("===")
            return {'bid': current_book['bid'].first.price, 'ask': current_book['ask'].first.price}
        return None

    def spread_volume(self, signal_threshold: int = 0) -> dict or None:
        """
        :return: {'bid': float, 'ask': float}
        """
        # TODO: add iteration to spread
        ...
        if current_book['bid'] and current_book['ask']:
            # logging.Logger.info(f"===[{self.iteration} - {signal_threshold}]===")
            # logging.Logger.info(
            #     f'bid: {self.order_book[self.iteration]["bid"].first.qty}, ask: {self.order_book[self.iteration]["ask"].first.qty}')
            # logging.Logger.info(
            #     f'bid: {current_book["bid"].first.qty}, ask: {current_book["ask"].first.qty}')
            # logging.Logger.info("===")
            return {'bid': current_book['bid'].first.qty, 'ask': current_book['ask'].first.qty}
        return None

    def price(self, signal_threshold: int = 0) -> float or None:
        spread = self.spread(signal_threshold=signal_threshold)
        if spread:
    ...
    def limit_order(self, order: Order):
        ...
        if order.order_type == 'bid':
            if order.price >= ask:
                order = self.order_book[self.iteration]['ask'].fulfill(order, t_cost)
            if order.qty > 0:
                self.order_book[self.iteration]['bid'].insert(order)
            return

        elif order.order_type == 'ask':
            if order.price <= bid:
                order = self.order_book[self.iteration]['bid'].fulfill(order, t_cost)
            if order.qty > 0:
                self.order_book[self.iteration]['ask'].insert(order)
    ...
    def cancel_order(self, order: Order):
        ...
        """
        if order.order_type == 'bid':
            self.order_book[self.iteration]['bid'].remove(order)
        elif order.order_type == 'ask':
            self.order_book[self.iteration]['ask'].remove(order)


class Trader:
    id = 0

    def __init__(self, market: ExchangeAgent, cash: float or int, assets: int = 0, signal_threshold: int = 0):
        """
    ...
    def __str__(self) -> str:
        return f'{self.name} ({self.type}) [{self.signal_threshold}]'

    def equity(self):
        if not self.signal_threshold:
            price = self.market.price() if self.market.price() is not None else 0
            return self.cash + self.assets * price
        price = self.market.price(signal_threshold=self.signal_threshold) if self.market.price(
            signal_threshold=self.signal_threshold
        ) is not None else 0
        return self.cash + self.assets * price

    def _buy_limit(self, quantity, price):
        order = Order(round(price, 1), round(quantity), 'bid', self.market.iteration, self)
        self.orders.append(order)
        self.market.limit_order(order)

    def _sell_limit(self, quantity, price):
        order = Order(round(price, 1), round(quantity), 'ask', self.market.iteration, self)
        self.orders.append(order)
        self.market.limit_order(order)
    ...
    def _buy_market(self, quantity) -> int:
        ...
        """
        if not self.market.order_book[self.iteration - self.signal_threshold]['ask']:
            return quantity
        order = Order(self.market.order_book[self.iteration - self.signal_threshold]['ask'].last.price, round(quantity),
                      'bid',
                      self.market.iteration, self)
    ...
    def _sell_market(self, quantity) -> int:
        ...
        :return: quantity unfulfilled
        """
        if not self.market.order_book[self.iteration - self.signal_threshold]['bid']:
            return quantity
        order = Order(self.market.order_book[self.iteration - self.signal_threshold]['bid'].last.price, round(quantity),
                      'ask',
                      self.market.iteration, self)
        return self.market.market_order(order).qty
...
class Random(Trader):
    ...
    def call(self):
        spread = self.market.spread(self.signal_threshold)
        if spread is None:
            return
...
class Fundamentalist(Trader):
    ...
    def call(self):
        ...
        p = self.market.price(self.signal_threshold)
        spread = self.market.spread(self.signal_threshold)
...
class Chartist(Trader):
    ...
    def change_sentiment(self, info, a1=1, a2=1, v1=.1):
        ...
        p = self.market.price(self.signal_threshold)  # market price
