...
from AgentBasedModel.utils import OrderList
...
class Simulator:
    ...
    def __init__(self, exchange: ExchangeAgent = None, traders: list = None, events: list = None):
        ...
        self.info = SimulatorInfo(self.exchange, self.traders, events)  # links to existing objects
    ...
    def simulate(self, n_iter: int, silent=False) -> object:
        for it in tqdm(range(n_iter), desc='Simulation', disable=silent):
            ...
            self.exchange.order_book[it + 1] = self.exchange.order_book[it]
            self.exchange.order_book[it] = {
                'bid': OrderList.from_list([self.exchange.order_book[it]['bid'].first]),
                'ask': OrderList.from_list([self.exchange.order_book[it]['ask'].first])
            }
...
class SimulatorInfo:
    ...
    def capture(self):
        ...
        *Traders Statistics*
        ...
        self.orders.append({
            'quantity': {'bid': len(self.exchange.order_book[self.exchange.iteration]['bid']),
                         'ask': len(self.exchange.order_book[self.exchange.iteration]['ask'])},
            # 'price mean': {
